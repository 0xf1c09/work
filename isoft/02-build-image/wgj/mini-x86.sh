...
mkdir -p $targetdir
debootstrap --no-check-gpg --variant=minbase buster $targetdir http://10.15.0.120:8087
...
chroot $targetdir sh -c 'echo "deb [trusted=yes] http://10.15.0.120:8087 buster main" > /etc/apt/sources.list'
