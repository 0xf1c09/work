...
static void set_phone_item(phone *phone_item)
{
	...
	phone_item->id = 1;
	snprintf(phone_item->name, NAME_STR_LEN, "%s", "libxml2-dev");
	snprintf(phone_item->tel, TEL_STR_LEN, "%s", "100 0000 0000");
	snprintf(phone_item->address, ADDR_STR_LEN, "%s", "Beijing");
}
